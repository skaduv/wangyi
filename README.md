...
请先进入 Git 仓库根目录再运行，避免误用其他目录中的旧脚本副本：
...
- **每支广告观看并完成有效点击停留后领取 1 次权益** — 点击后等待时间取广告动态 `clickStayTime`（最低保留 3 秒兼容下限）
- **领取成功后主动刷新状态** — 约 1 秒后刷新 stage 与 free-listen 数据，便于区分“未发放”和“已发放但页面未刷新”
...
[3/4] 上报点击...
  点击: code=200
  点击后停留 6 秒 (按广告 clickStayTime 动态取值)...
[4/4] 领取权益...
...
- `b_tag_state.json` 由 workflow 的 actions/cache 缓存：每次运行用上海日期和
  `github.run_id` 与 `github.run_attempt` 创建唯一保存键，后续运行按同日键前缀恢复最近状态，保证不可变
  cache 也能持续保存最新轮换索引；脚本按文件内日期自动跨天重置
...
- 当前未实现 `/ad/monitor/incentive/video` 播放阶段事件。现有抓包不足以确认其完整字段与时序，因此不会猜测或伪造该请求；若动态停留和会话关联修复后仍仅部分广告失败，需要补充脱敏抓包后再实现。
